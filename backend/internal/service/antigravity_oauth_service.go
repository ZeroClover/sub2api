...
func (s *AntigravityOAuthService) ExchangeCode(ctx context.Context, input *AntigravityExchangeCodeInput) (*AntigravityTokenInfo, error) {
	...
	// 获取 project_id 和 tier_id（部分账户类型可能没有），失败时重试
	projectID, tierID, loadErr := s.loadProjectInfoWithRetry(ctx, tokenResp.AccessToken, proxyURL, 3)
	if loadErr != nil {
...
func (s *AntigravityOAuthService) ValidateRefreshToken(ctx context.Context, refreshToken string, proxyID *int64) (*AntigravityTokenInfo, error) {
	...
	// 获取 project_id 和 tier_id（容错，失败不阻塞）
	projectID, tierID, loadErr := s.loadProjectInfoWithRetry(ctx, tokenInfo.AccessToken, proxyURL, 3)
	if loadErr != nil {
		fmt.Printf("[AntigravityOAuth] 警告: 获取 project_id 失败（重试后）: %v\n", loadErr)
...
func (s *AntigravityOAuthService) RefreshAccountToken(ctx context.Context, account *Account) (*AntigravityTokenInfo, error) {
	...
	// 每次刷新都调用 LoadCodeAssist 获取 project_id 和 tier_id，失败时重试
	existingProjectID := strings.TrimSpace(account.GetCredential("project_id"))
	existingTierID := strings.TrimSpace(account.GetCredential("tier_id"))
	projectID, tierID, loadErr := s.loadProjectInfoWithRetry(ctx, tokenInfo.AccessToken, proxyURL, 3)

	if loadErr != nil {
		// LoadCodeAssist 失败，保留原有 project_id 和 tier_id
...
// loadProjectInfoWithRetry 带重试机制获取 project_id 和 tier_id
// 返回 (project_id, tier_id, error)，失败时会重试指定次数
func (s *AntigravityOAuthService) loadProjectInfoWithRetry(ctx context.Context, accessToken, proxyURL string, maxRetries int) (string, string, error) {
	var lastErr error
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if err == nil && loadResp != nil && loadResp.CloudAICompanionProject != "" {
			return loadResp.CloudAICompanionProject, loadResp.GetTier(), nil
		}

		if err == nil {
			// 提取 tier（即使 onboard 也能拿到）
			var tierID string
			if loadResp != nil {
				tierID = loadResp.GetTier()
			}
...
func (s *AntigravityOAuthService) FillProjectID(ctx context.Context, account *Account, accessToken string) (string, error) {
	...
	projectID, _, err := s.loadProjectInfoWithRetry(ctx, accessToken, proxyURL, 3)
